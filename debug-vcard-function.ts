...
serve(async (req) => {
  ...
  try {
    ...
    console.log('Template ID:', card.template_id);
    console.log('Template data:', card.templates);
    console.log('Template elements:', card.templates?.elements);
    console.log('Social links:', card.social_links);
    console.log('==================');
    ...
    templateElements.forEach((element: any) => {
      ...
      console.log(`Processing element: ${fieldId}, type: ${fieldType}, value: ${fieldValue}`);
    ...
    // เพิ่มข้อมูลจาก field_values ที่ยังไม่ได้ประมวลผล
    Object.keys(fieldValues).forEach(key => {
      const value = fieldValues[key];
      if (!value || value.trim() === '') return;
      
      console.log(`Processing field_values: ${key} = ${value}`);
      
      // ข้ามฟิลด์ที่ประมวลผลแล้ว
      const processedFields = ['name', 'job_title', 'company', 'phone', 'email', 'address', 
                              'website', 'linkedin', 'facebook', 'twitter', 'instagram'];
      if (processedFields.includes(key)) return;
      
      // เพิ่มฟิลด์อื่นๆ เป็น NOTE
      const fieldName = key.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase());
      vcard += `NOTE:${fieldName}: ${value}\n`;
    });
    
    vcard += 'END:VCARD';

    console.log('Final vCard:', vcard);
    ...
    await supabaseClient
      ...
      .insert({
        ...
        device_info: 'vCard Generated (Debug)',
      });
