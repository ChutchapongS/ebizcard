        ...
        try {
          ...
          if (result.success && result.profile) {
            const profile = result.profile;
              ...
              const userDataToSet = {
                name: profile.full_name || user.user_metadata?.full_name || 'John Doe',
                nameEn: profile.full_name_english || user.user_metadata?.full_name_english || '',
                personalId: profile.personal_id || user.user_metadata?.personal_id || '',
                personalPhone: profile.personal_phone || user.user_metadata?.personal_phone || '',
                personalEmail: user.email || '',
                workName: profile.company || user.user_metadata?.company || '',
                workDepartment: profile.department || user.user_metadata?.department || '',
                workPosition: profile.job_title || user.user_metadata?.job_title || '',
                workPhone: profile.work_phone || user.user_metadata?.work_phone || '',
                workEmail: profile.work_email || user.user_metadata?.work_email || '',
                workWebsite: profile.website || user.user_metadata?.website || '',
                taxIdMain: profile.tax_id_main || user.user_metadata?.tax_id_main || '',
                taxIdBranch: profile.tax_id_branch || user.user_metadata?.tax_id_branch || '',
                address: addressData || user.user_metadata?.address || '',
                addresses: addressesList, // เพิ่ม addresses list
                // เพิ่มข้อมูลรูปภาพ
                avatar_url: profile.avatar_url || user.user_metadata?.avatar_url || '',
                profileImage: profile.avatar_url || user.user_metadata?.avatar_url || '',
                companyLogo: profile.company_logo || user.user_metadata?.company_logo || '',
                logo_url: profile.company_logo || user.user_metadata?.company_logo || '',
                user_metadata: {
                  avatar_url: profile.avatar_url || user.user_metadata?.avatar_url || '',
                  company_logo: profile.company_logo || user.user_metadata?.company_logo || '',
                  personal_id: profile.personal_id || user.user_metadata?.personal_id || '',
                  tax_id_main: profile.tax_id_main || user.user_metadata?.tax_id_main || '',
                  tax_id_branch: profile.tax_id_branch || user.user_metadata?.tax_id_branch || '',
                  personal_address_1_id: profile.personal_address_1_id || user.user_metadata?.personal_address_1_id || '',
                  personal_address_2_id: profile.personal_address_2_id || user.user_metadata?.personal_address_2_id || '',
                  work_address_1_id: profile.work_address_1_id || user.user_metadata?.work_address_1_id || '',
                  work_address_2_id: profile.work_address_2_id || user.user_metadata?.work_address_2_id || ''
                },
                // เพิ่มข้อมูล user_plan
                user_plan: profile.user_plan || 'Free',
                facebook: profile.facebook || user.user_metadata?.facebook || '',
                lineId: profile.line_id || user.user_metadata?.line_id || '',
                linkedin: profile.linkedin || user.user_metadata?.linkedin || '',
                twitter: profile.twitter || user.user_metadata?.twitter || '',
                instagram: profile.instagram || user.user_metadata?.instagram || '',
                tiktok: profile.tiktok || user.user_metadata?.tiktok || '',
                youtube: profile.youtube || user.user_metadata?.youtube || '',
                telegram: profile.telegram || user.user_metadata?.telegram || '',
                whatsapp: profile.whatsapp || user.user_metadata?.whatsapp || '',
                wechat: profile.wechat || user.user_metadata?.wechat || '',
                snapchat: profile.snapchat || user.user_metadata?.snapchat || '',
                pinterest: profile.pinterest || user.user_metadata?.pinterest || '',
                reddit: profile.reddit || user.user_metadata?.reddit || '',
                discord: profile.discord || user.user_metadata?.discord || '',
                slack: profile.slack || user.user_metadata?.slack || '',
                viber: profile.viber || user.user_metadata?.viber || '',
                skype: profile.skype || user.user_metadata?.skype || '',
                zoom: profile.zoom || user.user_metadata?.zoom || '',
                github: profile.github || user.user_metadata?.github || '',
                twitch: profile.twitch || user.user_metadata?.twitch || ''
              };
  ...
  const handleSaveTemplate = async () => {
    try {
      ...
      setTemplates(prev => [...prev, newTemplate]);
        ...
        // Redirect to card-editor with the new template
        window.location.href = `/card-editor?templateId=${result.data.id}`;
        return;
      }
      ...
      console.error('Error saving template:', error);
        alert('เกิดข้อผิดพลาดในการบันทึก แบบนามบัตร');
        setIsSaving(false);
    }
  };
            ...
            {loadingTemplates ? (
              ...
              <>
                ...
                {templates
                  ...
                  .map((template) => (
                        <div key={template.id} className={`rounded-lg shadow-md p-3 sm:p-4 hover:shadow-lg transition-shadow h-80 sm:h-96 flex flex-col relative ${
                      ...
                      <button
                        onClick={() => {
                          ...
                          setElements(template.elements);
                                ...
                                if (paperSettings.background?.type === 'image' && paperSettings.background?.image) {
                                ...
                                } else if (template.previewImage && template.previewImage.startsWith('data:')) {
                                  // Use preview image as background if no background image is set
                                  ...
                                  console.log('Using preview image as background:', template.previewImage.substring(0, 50) + '...');
                                }
