...
export const getDistricts = (province: string): string[] => {
  if (!province || !(address as any)[province]) {
    return [];
  }
  return Object.keys((address as any)[province]);
};
...
export const getTambons = (province: string, district: string): string[] => {
  if (!province || !district || !(address as any)[province] || !(address as any)[province][district]) {
    return [];
  }
  return (address as any)[province][district].Tambons || [];
};
...
export const getPostalCode = (province: string, district: string): number | null => {
  ...
  return (address as any)[province][district].PostCode || null;
};
...
export const getAddressData = (province: string, district: string): DistrictData | null => {
  ...
  const districtData = (address as any)[province][district];
  return {
    name: district,
...
export const searchByPostalCode = (postalCode: number): AddressData[] => {
  ...
  Object.keys(address).forEach(province => {
    Object.keys((address as any)[province]).forEach(district => {
      const districtData = (address as any)[province][district];
...
export const getAllDistrictsWithTambons = (province: string): DistrictData[] => {
  ...
  return Object.keys((address as any)[province]).map(district => ({
    name: district,
