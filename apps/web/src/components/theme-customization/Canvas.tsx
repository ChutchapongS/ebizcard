...
export function Canvas({
  ...
  const QRCodeCanvas = ({ element, content }: { element: CanvasElement; content: string }) => {
    ...
    useEffect(() => {
      if (qrRef.current) {
        ...
        if (element.qrLogo) {
          console.log('🔍 Canvas: Adding logo to QR Code');
          console.log('  - qrLogo:', element.qrLogo);
          ...
          console.log('  - qrOptions.imageOptions:', qrOptions.imageOptions);
        } else {
          console.log('🔍 Canvas: No logo found for QR Code');
        }
  ...
  const getFieldValue = (field: string, useAddressPrefix: boolean = true): string => {
    if (!userData || !field) {
        return '';
    }
  ...
  const getElementContent = (element: CanvasElement): string => {
    if (element.type === 'qrcode') {
      return element.content || 'https://www.scgjwd.com/';
    }
    // If element has field binding, use field value (ignore content)
    if (element.field) {
      const isAddressField = ['personalAddress1', 'personalAddress2', 'workAddress1', 'workAddress2'].includes(element.field);
      return getFieldValue(element.field, isAddressField ? element.useAddressPrefix !== false : true);
    }
                    ...
                    {element.type === 'picture' && (element.imageUrl || element.field) ? (
                      ...
                      <div className="w-full h-full flex items-center justify-center">
                        ...
                        {element.type === 'social' ? (
                          ...
                          >
                            {(() => {
                              const value = getFieldValue(element.field || '', ['personalAddress1','personalAddress2','workAddress1','workAddress2'].includes(element.field || '') ? (element.useAddressPrefix !== false) : true);
                      ...
                      <div className="w-full h-full flex items-center justify-center">
                        <QRCodeCanvas
                          element={element}
                          content={getElementContent(element) || 'https://www.scgjwd.com/'}
                        />
                      </div>
                    ) : (
                      <div
